...
repeat_passwd.onChange = function () {
    register_btn.disabled = true
    personal_data.checked = false
}

register_btn.onclick = function () {
    if (lv1_text.className === 'validated' &&
        lv2_text.className === 'validated' &&
        pv1_text.className === 'validated' &&
        pv2_text.className === 'validated' &&
        pv3_text.className === 'validated' &&
        ...
        let preregistationSerialized = JSON.stringify(preregistation);
        localStorage.setItem("preregistation", preregistationSerialized);
    }
};

this.onload = function () {
    let preregistationDeserialized = JSON.parse(localStorage.getItem("preregistation"));
    name.value = preregistationDeserialized.name;
    surname.value = preregistationDeserialized.surname;
    login.value = preregistationDeserialized.login;
    email.value = preregistationDeserialized.e_mail;
    password.value = preregistationDeserialized.passwd;
};
